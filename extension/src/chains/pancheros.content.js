// Content script for *.pancheros.com. Same belt-and-suspenders scanner
// pattern as the other chains. Pancheros calls their currency "Pancheros
// Rewards points"; the scanners look for "points" / "rewards" copy and
// fall through API → semantic selectors → label proximity.
...
(function () {
  ...
  const API_CANDIDATES = [
    "https://www.pancheros.com/api/customer/me",
    "https://www.pancheros.com/api/loyalty/balance",
    "https://www.pancheros.com/api/rewards/balance",
  ];
  ...
  const LABEL_RE =
    /\b(pancheros\s*rewards?\s*points?|rewards?\s*points?|points?\s*balance|your\s*points?|available\s*points?|total\s*points?|my\s*points?)\b/i;
  ...
  const TESTID_HINT = /(?:loyalty|points|balance|rewards|pancheros)/i;
  ...
  const LABELS = [
    ...
    /\bpancheros\s*rewards?\s*points?\b/i,
  ];
  ...
  async function attempt() {
    ...
    if (Date.now() - startedAt > MAX_RUNTIME_MS) {
      fail("Couldn't find your points. Make sure you're signed in and on pancheros.com/rewards.");
    }
  }

  try {
    chrome?.runtime?.onMessage?.addListener((msg, _sender, sendResponse) => {
      if (msg?.type !== "RESCAN" || msg.slug !== "pancheros") return;
      (async () => {
        if (ON_SIGNIN) {
  ...
  if (ON_SIGNIN) {
    safeSend({ type: "CHAIN_SIGNIN_DETECTED", slug: "pancheros" });
    return;
  }
