...
- [x] Unit test for `isExtensionConfigured` in `src/lib/extension-bridge.ts` — covers both the unset (false) and set (true) cases via `vi.resetModules()` + `vi.stubEnv` in `tests/unit/extension-bridge-lib.test.ts`.
