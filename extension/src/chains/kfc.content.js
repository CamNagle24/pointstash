// Content script for *.kfc.com. Same belt-and-suspenders scanner pattern
// as the other chains.
...
(function () {
  ...
  const API_CANDIDATES = [
    "https://www.kfc.com/api/customer/me",
    "https://www.kfc.com/api/loyalty/balance",
    "https://www.kfc.com/api/rewards/balance",
  ];
  ...
  // Confirmed on /dashboard: header value lives in
  // `<span data-testid="Header-points-value">N</span>`. The data-testid is
  // a stable hook across builds (unlike the emotion `SideNavBarHeader_*`
  // class hash next to it).
  function tryHighConfidence() {
    const el = document.querySelector('[data-testid="Header-points-value"]');
    if (!el) return null;
  ...
  const LABEL_RE =
    /\b(kfc\s*rewards?\s*points?|rewards?\s*points?|points?\s*balance|your\s*points?|available\s*points?|total\s*points?|my\s*points?)\b/i;
  ...
  const TESTID_HINT = /(?:loyalty|points|balance|rewards|kfc)/i;
  ...
  async function attempt() {
    ...
    if (Date.now() - startedAt > MAX_RUNTIME_MS) {
      fail("Couldn't find your points. Make sure you're signed in to KFC Rewards.");
    }
  }

  try {
    chrome?.runtime?.onMessage?.addListener((msg, _sender, sendResponse) => {
      if (msg?.type !== "RESCAN" || msg.slug !== "kfc") return;
      (async () => {
        if (ON_SIGNIN) {
  ...
  if (ON_SIGNIN) {
    safeSend({ type: "CHAIN_SIGNIN_DETECTED", slug: "kfc" });
    return;
  }
