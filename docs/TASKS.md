...
- [x] API integration test for `GET /api/cron/deal-reminders` — no opted-in users → zero stats; per-user email error captured in errors[] without aborting batch; extended `tests/unit/api-deal-reminders-cron.test.ts`.
