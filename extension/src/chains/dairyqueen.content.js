// Content script for *.dairyqueen.com. Same belt-and-suspenders scanner
// pattern as the other chains. DQ calls their currency "DQ Rewards points".
...
(function () {
  ...
  const API_CANDIDATES = [
    "https://www.dairyqueen.com/api/customer/me",
    "https://www.dairyqueen.com/api/loyalty/balance",
    "https://www.dairyqueen.com/api/rewards/balance",
  ];
  ...
  // Confirmed on /en-us/account/rewards/: the points value lives in an
  // `<h3 data-cy="rewardPointsAccordion-points">N</h3>`. Running first so a
  // looser scanner can't beat it to the punch.
  function tryHighConfidence() {
    const el = document.querySelector('[data-cy="rewardPointsAccordion-points"]');
    if (!el) return null;
    const text = (el.textContent || "").trim();
    const m = text.match(/^(\d{1,3}(?:,\d{3})*|\d+)$/);
    if (!m) return null;
    const n = parseGroupedInt(m[1]);
    return n != null ? { balance: n } : null;
  }

  const LABEL_TAGS = ["h2", "h3", "h4", "h5", "h6", "p", "span"];
  const VALUE_TAGS = ["h1", "h2", "h3"];
  const LABEL_RE =
    /\b(dq\s*rewards?\s*points?|rewards?\s*points?|points?\s*balance|your\s*points?|available\s*points?|total\s*points?|my\s*points?)\b/i;
  ...
  const TESTID_HINT = /(?:loyalty|points|balance|rewards|dq)/i;
  ...
  async function attempt() {
    ...
    if (Date.now() - startedAt > MAX_RUNTIME_MS) {
      fail("Couldn't find your points. Make sure you're signed in to DQ Rewards.");
    }
  }

  try {
    chrome?.runtime?.onMessage?.addListener((msg, _sender, sendResponse) => {
      if (msg?.type !== "RESCAN" || msg.slug !== "dairyqueen") return;
      (async () => {
        if (ON_SIGNIN) {
  ...
  if (ON_SIGNIN) {
    safeSend({ type: "CHAIN_SIGNIN_DETECTED", slug: "dairyqueen" });
    return;
  }
