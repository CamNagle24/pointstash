...
All your fast-food rewards points in one dashboard. Tracks balances across 17 chains
(McDonald's, Chick-fil-A, Wendy's, Taco Bell, Burger King, Popeyes, Subway, Dunkin',
Starbucks, Chipotle, Pancheros, Dairy Queen, Culver's, Jimmy John's, Buffalo Wild Wings,
KFC, Panda Express) and surfaces the highest-value redemptions in **cents-per-point**.
<!-- chain count = Object.keys(CHAINS).length in src/lib/constants.ts — update there first -->
...
npm install
npm run db:push
npm run db:seed              # 17 chains + 50+ redemptions
...
## Key paths
- `src/app/` routes (+ `api/`), `src/lib/scrapers/` (BaseScraper + bespoke scrapers + registry),
