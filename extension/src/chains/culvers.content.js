// Content script for *.culvers.com. Same belt-and-suspenders scanner
...
(function () {
  ...
  const API_CANDIDATES = [
    "https://www.culvers.com/api/customer/me",
    "https://www.culvers.com/api/loyalty/balance",
    "https://www.culvers.com/api/rewards/balance",
  ];
  ...
  // Confirmed on /rewards: header points live in a paragraph whose class
  // begins with the CSS-modules prefix `RewardsCenterHeader_pointsText__`
  // and reads e.g. "0 PTS". The hash suffix changes per build, so match by
  // prefix.
  function tryHighConfidence() {
    const nodes = document.querySelectorAll('[class*="RewardsCenterHeader_pointsText" i]');
  ...
  const LABEL_RE =
    /\b(myc\s*rewards?\s*points?|culver'?s\s*rewards?\s*points?|rewards?\s*points?|points?\s*balance|your\s*points?|available\s*points?|total\s*points?|my\s*points?)\b/i;
  ...
  const TESTID_HINT = /(?:loyalty|points|balance|rewards|culvers|myc)/i;
  ...
  async function attempt() {
    ...
    if (Date.now() - startedAt > MAX_RUNTIME_MS) {
      fail("Couldn't find your points. Make sure you're signed in to MyCulver's Rewards.");
    }
  }

  try {
    chrome?.runtime?.onMessage?.addListener((msg, _sender, sendResponse) => {
      if (msg?.type !== "RESCAN" || msg.slug !== "culvers") return;
      (async () => {
        if (ON_SIGNIN) {
  ...
  if (ON_SIGNIN) {
    safeSend({ type: "CHAIN_SIGNIN_DETECTED", slug: "culvers" });
    return;
  }
