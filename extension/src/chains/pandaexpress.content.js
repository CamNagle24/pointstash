// Content script for *.pandaexpress.com. Same belt-and-suspenders scanner
// pattern as the other chains.

(function () {
  const MAX_RUNTIME_MS = 45_000;
  const POLL_INTERVAL_MS = 1500;
  const FETCH_TIMEOUT_MS = 4_000;
  // Panda's rewards UI is an Angular SPA: the ".animation-points" balance
  // renders late, and the page also shows a "next reward" goal number (e.g.
  // 200) that the fuzzy scanners would happily grab first. Give the precise
  // high-confidence selector (and the API) this long to win before we fall
  // back to structural/aria/label scanning.
  const HC_GRACE_MS = 8_000;
  ...
  const API_CANDIDATES = [
    "https://www.pandaexpress.com/api/customer/me",
    "https://www.pandaexpress.com/api/loyalty/balance",
    "https://www.pandaexpress.com/api/rewards/balance",
  ];
  ...
  // High-confidence selector. On /myrewards/earn the balance lives in
  //   <div class="animation-points">
  //     <span class="points">124</span>
  //     <span class="points-label">Panda Points®</span>
  //   </div>
  // ".points"/".points-label" are generic, so anchor on the container that
  // also holds a ".points-label" matching "Panda Points" before trusting the
  // ".points" digits.
  // The counter animates 0 -> real value, and there can be more than one
  // ".animation-points" node (e.g. an aria-hidden duplicate). Take the max
  // label-verified value so a mid-animation/placeholder 0 never wins over the
  // settled balance. Callers decide whether a bare 0 is "not ready yet".
  function tryHighConfidence() {
    const containers = document.querySelectorAll(".animation-points");
    let best = null;
    for (const container of containers) {
      const labelEl = container.querySelector(".points-label");
      if (!labelEl || !/points/i.test(labelEl.textContent || "")) continue;
      const valEl = container.querySelector(".points");
      if (!valEl) continue;
    ...
    return best == null ? null : { balance: best };
  }

  const LABEL_TAGS = ["h2", "h3", "h4", "h5", "h6", "p", "span"];
  const VALUE_TAGS = ["h1", "h2", "h3"];
  const LABEL_RE =
    /\b(panda\s*rewards?\s*points?|panda\s*express\s*rewards?\s*points?|rewards?\s*points?|points?\s*balance|your\s*points?|available\s*points?|total\s*points?|my\s*points?)\b/i;
  ...
  const TESTID_HINT = /(?:loyalty|points|balance|rewards|panda)/i;
  ...
  async function attempt() {
    if (resolved) return;
    const withinGrace = Date.now() - startedAt < HC_GRACE_MS;
    const hcHit = tryHighConfidence();
    // A bare 0 during the grace window is almost certainly the counter still
    // animating up from zero — keep polling for the settled value.
    if (hcHit && !(withinGrace && hcHit.balance === 0)) {
    ...
    // Hold off on the fuzzy scanners until the precise selector has had a fair
    // shot — they can otherwise latch onto the on-screen "goal" number.
    ...
    if (Date.now() - startedAt > MAX_RUNTIME_MS) {
      fail("Couldn't find your points. Make sure you're signed in to Panda Rewards.");
    }
  }

  try {
    chrome?.runtime?.onMessage?.addListener((msg, _sender, sendResponse) => {
      if (msg?.type !== "RESCAN" || msg.slug !== "pandaexpress") return;
      (async () => {
        if (ON_SIGNIN) {
  ...
  if (ON_SIGNIN) {
    safeSend({ type: "CHAIN_SIGNIN_DETECTED", slug: "pandaexpress" });
    return;
  }
