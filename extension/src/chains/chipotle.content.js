// Content script for *.chipotle.com. Same belt-and-suspenders strategy as
// the other chains. Chipotle calls their currency "Chipotle Rewards points"
// (10 points per dollar) and balances typically run into the hundreds or
// thousands, so the label/value scanners look for "points" / "rewards" copy.
...
(function () {
  ...
  const API_CANDIDATES = [
    "https://www.chipotle.com/api/customer/me",
    "https://www.chipotle.com/api/loyalty/balance",
    "https://www.chipotle.com/api/rewards/balance",
  ];
  ...
  // Confirmed on /order/rewards: `<div class="num-points-text fs-unmask">N
  // POINTS</div>`. The leading integer is the user's balance. Running first
  // so a more brittle scanner can't beat it to the punch.
  function tryHighConfidence() {
    const nodes = document.querySelectorAll(".num-points-text");
    for (const el of nodes) {
      const text = (el.textContent || "").trim();
      const m = text.match(/^(\d{1,3}(?:,\d{3})*|\d+)\s*(?:pts?|points?)?$/i);
      if (!m) continue;
  ...
  // Intentionally requires "points" after "chipotle rewards" — the bare
  // brand name shows up on every marketing tile.
  const LABEL_RE =
    /\b(chipotle\s*rewards?\s*points?|rewards?\s*points?|points?\s*balance|your\s*points?|available\s*points?|total\s*points?|my\s*points?)\b/i;
  ...
  const TESTID_HINT = /(?:loyalty|points|balance|rewards|chipotle)/i;
  ...
  const LABELS = [
    ...
    /\bchipotle\s*rewards?\s*points?\b/i,
  ];
  ...
  async function attempt() {
    ...
    if (Date.now() - startedAt > MAX_RUNTIME_MS) {
      fail("Couldn't find your points. Make sure you're signed in and on chipotle.com/rewards.");
    }
  }

  try {
    chrome?.runtime?.onMessage?.addListener((msg, _sender, sendResponse) => {
      if (msg?.type !== "RESCAN" || msg.slug !== "chipotle") return;
      (async () => {
        if (ON_SIGNIN) {
  ...
  if (ON_SIGNIN) {
    safeSend({ type: "CHAIN_SIGNIN_DETECTED", slug: "chipotle" });
    return;
  }
