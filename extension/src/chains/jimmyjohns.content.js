// Content script for *.jimmyjohns.com. Same belt-and-suspenders scanner
...
(function () {
  ...
  const API_CANDIDATES = [
    "https://www.jimmyjohns.com/api/customer/me",
    "https://www.jimmyjohns.com/api/loyalty/balance",
    "https://www.jimmyjohns.com/api/rewards/balance",
  ];
  ...
  // Confirmed on /rewards: a MUI Box wraps a value div + a typography label
  // reading exactly "Points". The emotion `css-*` hashes rotate per build,
  // so anchor on `MuiTypography` + the literal label and then look for a
  // sibling MuiBox containing just digits.
  function tryHighConfidence() {
    const labels = document.querySelectorAll('p[class*="MuiTypography"]');
    for (const label of labels) {
      const text = (label.textContent || "").trim();
      if (!/^points$/i.test(text)) continue;
      const parent = label.parentElement;
      if (!parent) continue;
      const candidates = parent.querySelectorAll('div[class*="MuiBox-root"]');
  ...
  const LABEL_RE =
    /\b(freaky\s*fast\s*rewards?\s*points?|jimmy\s*john'?s\s*rewards?\s*points?|rewards?\s*points?|points?\s*balance|your\s*points?|available\s*points?|total\s*points?|my\s*points?)\b/i;
  ...
  const TESTID_HINT = /(?:loyalty|points|balance|rewards|freaky|jimmy)/i;
  ...
  const LABELS = [
    ...
    /\bfreaky\s*fast\s*rewards?\s*points?\b/i,
  ];
  ...
  async function attempt() {
    ...
    if (Date.now() - startedAt > MAX_RUNTIME_MS) {
      fail("Couldn't find your points. Make sure you're signed in to Freaky Fast Rewards.");
    }
  }

  try {
    chrome?.runtime?.onMessage?.addListener((msg, _sender, sendResponse) => {
      if (msg?.type !== "RESCAN" || msg.slug !== "jimmyjohns") return;
      (async () => {
        if (ON_SIGNIN) {
  ...
  if (ON_SIGNIN) {
    safeSend({ type: "CHAIN_SIGNIN_DETECTED", slug: "jimmyjohns" });
    return;
  }
